...
enum Exp {
    ...
    Rec { var: String, var_type: Type, value: Box<Exp>, body: Box<Exp> },
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Exp::Rec { var, var_type, value, body } => {
                write!(f, "(rec ({var}: {var_type}) {value} {body})")
            }
...
impl fmt::Display for Value {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Value::Fun { var, var_type, body, nv } => {
                write!(
                    f,
                    "closure((lambda ({var}: {var_type}) {body}), {nv:?})"
                )
            }
        }
    }
